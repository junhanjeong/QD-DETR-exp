#!/usr/bin/env python3
"""
Simplified Gate Analysis System
...
class SimplifiedGateAnalyzer:
    ...
    def analyze_gates(self, model):
        ...
        sample_rankings = self._calculate_sample_rankings(all_gate_data)
    ...
    def _collect_all_gate_data(self, gate_history):
        ...
        for sample_id, sample_data in gate_history.items():
            sample_gates = {
                'mha_gates': [],  # [layer][query] = scalar (global에서는 모든 query 동일)
                'ffn_gates': []   # [layer][query] = scalar (global에서는 모든 query 동일)
            }
            ...
            for layer_idx in range(len(sample_data['attention_gates'])):
                ...
                # Global gating: 첫 번째 query의 값만 사용 (모든 query가 동일하므로)
                mha_avg = float(mha_gate[0, 0, :].mean())  # 첫 번째 query의 평균
                ffn_avg = float(ffn_gate[0, 0, :].mean())  # 첫 번째 query의 평균
                
                sample_gates['mha_gates'].append(mha_avg)
                sample_gates['ffn_gates'].append(ffn_avg)
    ...
    def _calculate_layer_gate_averages(self, all_data):
        ...
        for layer_idx in range(layer_count):
            # 모든 샘플에서 해당 레이어의 게이트 값들 수집
            mha_values = []
            ffn_values = []
            
            for sample_id, sample_data in all_data['samples'].items():
                if layer_idx < len(sample_data['mha_gates']):
                    mha_values.append(sample_data['mha_gates'][layer_idx])
                    ffn_values.append(sample_data['ffn_gates'][layer_idx])
    ...
    def _calculate_sample_rankings(self, all_data):
        ...
        for sample_id, sample_data in all_data['samples'].items():
            mha_values = sample_data['mha_gates']  # 이미 레이어별 스칼라 값
            ffn_values = sample_data['ffn_gates']  # 이미 레이어별 스칼라 값
